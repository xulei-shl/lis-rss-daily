...
Usage:
    ...
    python gmail_skill.py refresh-auth [--account EMAIL]  # Force re-authentication
...
def cmd_refresh_auth(args):
    """Force re-authentication for an account and update stored tokens."""
    client_config = get_client_config()
    account_email = resolve_account_email(args.account)
    token_path = get_token_path(account_email or args.account)
    ...
    login_email = stored_email or account_email
    token_data = do_oauth_flow(client_config, login_hint=login_email, force_consent=True)

    token_path = get_token_path(token_data.get("email", args.account))
    with open(token_path, "w") as f:
        json.dump(token_data, f, indent=2)

    print(json.dumps({
        "success": True,
        "message": f"Re-authenticated: {token_data.get('email', 'unknown')}",
...
def main():
    ...
    refresh_auth_parser = subparsers.add_parser("refresh-auth", help="Force re-authentication and refresh stored tokens")
    add_account_arg(refresh_auth_parser)
    refresh_auth_parser.set_defaults(func=cmd_refresh_auth)
