...
本系统采用**分层兜底配置**设计，数据库配置与环境变量配置各司其职：

| 优先级 | 配置来源 | 说明 |
|--------|----------|------|
| 1 | 数据库配置 (`llm_configs` 表) | 用户业务配置，动态可修改 |
...
系统提示词用于控制 LLM 的行为，包括文章过滤、摘要生成、关键词提取等。
...
| 类型 | 用途 | 变量占位符 |
|------|------|-----------|
| `filter` | 文章过滤判断 | `{{topics}}`, `{{domains}}`, `{{user_keywords}}` |
| `summary` | 生成中文摘要 | `{{article_title}}`, `{{article_content}}` |
| `keywords` | 提取研究标签 | `{{article_title}}`, `{{article_content}}` |
| `translation` | 翻译处理 | `{{text}}` |

#### 兜底策略

- **运行时优先使用模板**: 按类型读取启用模板并渲染变量
- **缺失时回退**: 当对应类型模板不存在或为空时
  - `summary/keywords/translation` 回退内置提示词
  - `filter` 回退为 `topic_domains` + `topic_keywords` 动态拼装

### Chroma 向量库配置

在「设置」→「Chroma 设置」页面配置语义搜索：

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| Host | Chroma 服务器地址 | 127.0.0.1 |
| Port | Chroma 端口 | 8000 |
| Collection | 集合名称 | articles |
| 距离度量 | 相似度算法 | cosine |
...
| 模式 | 说明 | 适用场景 |
|------|------|----------|
| **混合搜索** | 向量 + 关键词融合（默认） | 日常搜索，推荐使用 |
| **语义搜索** | 纯向量检索 | 理解查询意图 |
| **关键词搜索** | 传统关键词匹配 | 精确匹配，性能最好 |
| **相关文章** | 基于文章推荐相似文章 | 浏览时发现相关内容 |
...
混合搜索默认启用，结合了语义检索和关键词搜索的优势：

- **语义权重 70%**: 向量相似度，理解查询意图
- **关键词权重 30%**: 精确匹配，确保关键词命中
- **智能回退**: 如果向量检索失败，自动回退到关键词搜索
...
### Q: 如何配置多个 LLM 配置实现故障转移？

**A**:
1. 进入「设置」→「LLM 配置」页面
2. 添加多条配置，每条设置不同的优先级
3. 将主配置设置为默认（is_default=1）
4. 当主配置调用失败时，系统会自动切换到下一优先级配置
