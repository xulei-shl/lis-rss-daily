...
interface SearchResult {
  ...
  metadata?: {
    ...
    source_origin?: 'rss' | 'journal' | 'keyword'; // 来源类型
    journal_name?: string; // 期刊名称
    keyword_name?: string; // 关键词订阅名称
  };
}
```
...
// 强制刷新缓存（useCache 参数会被忽略，refreshCache: true 时会重新计算并更新缓存）
const response3 = await search({
  mode: SearchMode.RELATED,
  userId: 1,
  articleId: 123,
  limit: 5,
  useCache: true,  // 此处值不影响，refreshCache: true 时会重新计算
  refreshCache: true,
});
```
