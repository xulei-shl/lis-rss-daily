...
当同一用户存在多条 LLM 配置时，调用顺序遵循以下规则：

1. **默认优先**：`is_default = 1` 的配置优先尝试  
2. **优先级排序**：在同一默认级别内，按 `priority` 升序（数字越小越优先）  
3. **创建时间**：若优先级相同，按 `created_at` 升序  

失败判定：调用异常或返回空响应会触发切换到下一条配置。

示例：
- 现有默认配置 A（priority=100）
- 新增非默认配置 B（priority=95）
调用顺序：先 A，A 失败后按优先级尝试 B。
