...
- 发送方法（都遍历 chats、**逐 chat 错误隔离**，一个失败不影响其他，`successCount>0` 即算成功）：
  - `sendDailySummary(userId, data)`（`:118`）→ `getDailySummaryChatsByType`
  - `sendJournalAllSummary`（`:185`）→ `getJournalAllChats`（type=`'journal_all'`）
  - `sendInsightsSummary`（`:257`）→ `getInsightsChats`（type=`'insights'`）
  - `sendNewArticle`（`:401`）→ `getNewArticlesChats`，带内联键盘 `sendMessageWithKeyboard`；摘要优先级 `summary_zh>summary>markdown_content>content`，预览截 500 字
  - `sendPdfSummary`（`:491`）→ `getPdfSummaryChats`
...
- ⚠️ **推送在 `daily-summary.ts` 内触发**，不在调度器：`generateDailySummary` 内 `getTelegramNotifier().sendDailySummary` + `getWeChatNotifier().sendDailySummary`（fire-and-forget）。调度器只负责生成 + 保存。
...
- 报告内容（`generateInsightsSummary`, `api/daily-summary.ts:952`）：取最近 `days`（默认 **15**）天 `filter_status='passed'`、来源在期刊白名单、含正文（排除 `%<正>%`）的文章（≤60），用 `insights` 系统提示词生成「1500–3000 字中文洞察报告」，按主题分组含研究趋势与选题建议，temp 0.3。
- 推送到 Telegram + 企业微信（`sendInsightsSummary`）并存 `daily_summaries`（`type='insights'`）。
...
6. **推送在 `daily-summary.ts`/insights 生成函数内触发**，调度器只生成+保存。
