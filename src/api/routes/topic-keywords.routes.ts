 ...
 */
router.post('/topic-keywords', requireAuth, requireUser, async (req: AuthRequest, res) => {
  try {
 ...
 */
router.put('/topic-keywords/:id', requireAuth, requireUser, async (req: AuthRequest, res) => {
  try {
    const idParam = req.params.id;
    if (Array.isArray(idParam)) {
      return res.status(400).json({ error: 'Invalid topic keyword ID' });
 ...
 */
router.delete('/topic-keywords/:id', requireAuth, requireUser, async (req: AuthRequest, res) => {
  try {
    const idParam = req.params.id;
    if (Array.isArray(idParam)) {
      return res.status(400).json({ error: 'Invalid topic keyword ID' });
