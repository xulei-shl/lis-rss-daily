  ...
  python rdfybk_spider.py -j G9 -y 2024 -i 6 --no-headless
    ...
    try:
        ...
        if args.sync:
            ...
            # 异步模式（默认）
            print(f"使用异步模式（并发数: {args.concurrency}）...", file=sys.stderr)
            if len(issues) == 1:
        ...
        if papers:
            ...
            if args.output == '-':
                ...
                print(f"\n[OK] 成功爬取 {len(papers)} 篇论文", file=sys.stderr)
        else:
            print("\n[WARN] 未找到任何论文", file=sys.stderr)
            # 输出空数组
            if args.output == '-':
                print('[]')

    except KeyboardInterrupt:
        print("\n\n[INTERRUPTED] 用户中断执行", file=sys.stderr)
        sys.exit(0)
    except Exception as e:
