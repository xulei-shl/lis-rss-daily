...
class WanfangDetailSpider:
    ...
    def _extract_abstract(self, page: SyncPage) -> str:
        ...
        # 先等待摘要元素出现
        try:
            page.wait_for_selector('.summary.list', timeout=5000)
        except:
            pass  # 继续尝试
    ...
    def _extract_publish_date(self, page: SyncPage) -> str:
        """
        提取发表日期
        """
        for selector in self.PUBLISH_DATE_SELECTORS:
            try:
                elem = page.locator(selector)
                if elem.count() > 0:
                    text = elem.first.inner_text()
...
class AsyncWanfangDetailSpider:
    """异步论文详情爬虫类 - 支持并发爬取"""

    # 选择器配置（复用同步版本）
    ABSTRACT_SELECTORS = WanfangDetailSpider.ABSTRACT_SELECTORS
    PUBLISH_DATE_SELECTORS = WanfangDetailSpider.PUBLISH_DATE_SELECTORS
    ...
    async def _extract_all_async(self, page: Page) -> Dict[str, Any]:
        ...
        return {
            ...
            "publish_date": await self._extract_publish_date_async(page),
        }
    ...
    async def _extract_publish_date_async(self, page: Page) -> str:
        """提取发表日期（异步版本）"""
        for selector in self.PUBLISH_DATE_SELECTORS:
            try:
                elem = page.locator(selector)
...
def get_paper_detail(page: SyncPage, url: str, timeout: int = 30000,
    ...
    Returns:
        论文详情字典
    """
    spider = WanfangDetailSpider(timeout=timeout, delay=delay)
...
if __name__ == "__main__":
    ...
    test_url = "https://d.wanfangdata.com.cn/periodical/zgtsgxb202502001"

    with Camoufox(headless=False) as browser:
        page = browser.new_page()

        spider = WanfangDetailSpider(timeout=30000)

        detail = spider.fetch_detail(page, test_url)
        print(f"摘要: {detail.get('abstract', '')[:200] if detail else 'None'}")
        print(f"发表日期: {detail.get('publish_date', '') if detail else 'None'}")
